...
def quiz():
    ...
    utils.clear_screen()
    utils.play_sound("lom.mp3", 0)
    time.sleep(2)
    question_lines = utils.open_file('questions.txt', "r")
    ...
    for i in range(15):
        ...
        while answer not in list(answers.keys()):
            if answer == "t":
                utils.play_sound("music_off.mp3", 0)
                answer = safe_input("\nSelect the correct answer ('a','b','c','d') ! ",
                                    ["a", "b", "c", "d"])
                time.sleep(2)
                utils.clear_screen()
                utils.play_sound("marked.mp3", 0)
                time.sleep(2)
        ...
        if is_correct:
            if i < 14:
                utils.play_sound("correct_answer.mp3", 0)
                if i == 4:
                    ...
                    utils.play_sound("now_comes_hard_part.mp3", 0)
                    time.sleep(1)
                else:
                ...
                utils.play_sound("great_logic.mp3", 0)
                time.sleep(1)
                utils.clear_screen()
                print(fg.purple + "Congratulations! You have won 40 000 000 Ft!" + fg.rs)
                utils.play_sound("winning_theme.mp3", 0)
                time.sleep(35)
                sys.exit(0)
        else:
            print(fg.red + "Bad answer! Better luck next time!" + fg.rs)
...
def print_phone_conversation(text, question, answers, good_answer):
    utils.play_sound("phone_ring.mp3", 0)
    time.sleep(2)
    utils.play_sound("phone_call.mp3", 0)
    then = time.time()
    ...
    print(fg.orange + str(30 - int(time.time() - then)) + fg.rs)
    now = time.time()
    utils.play_sound('phone_call.mp3', 30.0)
    time.sleep(3)
...
def telephone_help(question, answers, correct_answer):
    ...
    for i in range(len(call_text_files)):
        if phone.lower() == call_text_files[i][0]:
            text = (utils.open_file(call_text_files[i], 'r'))
            print_phone_conversation(text, question, answers, correct_answer)


def halving(question, answers, correct_answer):
    utils.play_sound("lets_take_two.mp3", 0)
    utils.clear_screen()
    time.sleep(2)
    utils.play_sound("halving.mp3", 0)
...
def audience_help(question, answers, correct_answer):
    utils.play_sound("push_your_buttons.mp3", 0)
    time.sleep(3)
    chance_of_answers = {}
    utils.clear_screen()
    for i in range(4):
